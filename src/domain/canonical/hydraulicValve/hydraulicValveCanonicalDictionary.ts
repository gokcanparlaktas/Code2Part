...
export const WAYS_POSITIONS_DICTIONARY: Record<
  ...
  CanonicalDictionaryEntry
> = {
  '2_2': entry('2_2', '2/2', 'critical', { aliases: ['2/2', '2-2', '2WAY2POS'] }),
  '3_2': entry('3_2', '3/2', 'critical', { aliases: ['3/2', '3-2', '3WAY2POS'] }),
  '4_2': entry('4_2', '4/2', 'critical', { aliases: ['4/2', '4-2', '4WAY2POS'] }),
  '4_3': entry('4_3', '4/3', 'critical', { aliases: ['4/3', '4-3', '4WAY3POS'] }),
  '5_2': entry('5_2', '5/2', 'critical', { aliases: ['5/2', '5-2', '5WAY2POS'] }),
  '5_3': entry('5_3', '5/3', 'critical', { aliases: ['5/3', '5-3', '5WAY3POS'] }),
};
...
> = {
  ...
  DC_48V: entry('DC_48V', '48V DC', 'critical', { aliases: ['D48', '48DC', 'DC48'] }),
  DC_110V: entry('DC_110V', '110V DC', 'critical', { aliases: ['110DC', 'DC110', 'D110'] }),
  DC_220V: entry('DC_220V', '220V DC', 'critical', { aliases: ['220DC', 'DC220'] }),
  AC_24V: entry('AC_24V', '24V AC', 'critical', { aliases: ['A24', '24AC', 'AC24'] }),
  AC_48V: entry('AC_48V', '48V AC', 'critical', { aliases: ['A48', '48AC', 'AC48'] }),
  AC_110V: entry('AC_110V', '110V AC', 'critical', { aliases: ['A110', '110AC', 'AC110'] }),
  AC_115V: entry('AC_115V', '115V AC', 'critical', { aliases: ['A115', '115AC', 'AC115'] }),
  AC_120V: entry('AC_120V', '120V AC', 'critical', { aliases: ['A120', '120AC', 'AC120'] }),
  AC_220V: entry('AC_220V', '220V AC', 'critical', { aliases: ['A220', '220AC', 'AC220'] }),
  AC_230V: entry('AC_230V', '230V AC', 'critical', { aliases: ['A230', '230AC', 'AC230'] }),
  AC_240V: entry('AC_240V', '240V AC', 'critical', { aliases: ['A240', '240AC', 'AC240'] }),
};
...
> = {
  ...
  PLUG_IN_CONNECTOR: entry('PLUG_IN_CONNECTOR', 'Takılı konnektör', 'important', {
    aliases: ['N', 'N1'],
  }),
};
...
> = {
  ...
  protected_manual_override: entry('protected_manual_override', 'Korumalı manuel kumanda', 'important'),
  concealed_manual_override: entry('concealed_manual_override', 'Gizli manuel kumanda', 'important'),
  detent_manual_override: entry('detent_manual_override', 'Detent manuel kumanda', 'important', {
    aliases: ['N9'],
  }),
};
...
export const FIELD_LABELS = {
  ...
  spoolFunctionCode: 'Sürgü / fonksiyon kodu',
} as const;
...
function buildAliasIndex<T extends string>(
  ...
  for (const [canonicalKey, dictEntry] of Object.entries(dictionary) as [T, CanonicalDictionaryEntry][]) {
...
export function getWaysPositionsDisplay(key: HydraulicValveWaysPositions | null): string {
  if (!key || key === 'unknown') {
    return 'Bilinmiyor';
  }
...
const CENTER_CONDITION_ALIAS_INDEX = buildAliasIndex(CENTER_CONDITION_DICTIONARY);
...
export function lookupMountingStandardAlias(token: string): HydraulicMountingStandard | null {
  return (MOUNTING_ALIAS_INDEX.get(compactToken(token)) as HydraulicMountingStandard | undefined) ?? null;
}

export function lookupWaysPositionsAlias(token: string): HydraulicValveWaysPositions | null {
  return (WAYS_POSITIONS_ALIAS_INDEX.get(compactToken(token)) as HydraulicValveWaysPositions | undefined) ?? null;
}

export function lookupCenterConditionAlias(token: string): HydraulicCenterCondition | null {
  return (CENTER_CONDITION_ALIAS_INDEX.get(compactToken(token)) as HydraulicCenterCondition | undefined) ?? null;
}

export function lookupCenteringAlias(token: string): HydraulicCentering | null {
  return (CENTERING_ALIAS_INDEX.get(compactToken(token)) as HydraulicCentering | undefined) ?? null;
}

export function lookupCoilVoltageAlias(token: string): CanonicalCoilVoltage | null {
  return (COIL_VOLTAGE_ALIAS_INDEX.get(compactToken(token)) as CanonicalCoilVoltage | undefined) ?? null;
}

export function lookupConnectorTypeAlias(token: string): CanonicalConnectorType | null {
  return (CONNECTOR_ALIAS_INDEX.get(compactToken(token)) as CanonicalConnectorType | undefined) ?? null;
}

export function lookupManualOverrideAlias(token: string): CanonicalManualOverride | null {
  return (MANUAL_OVERRIDE_ALIAS_INDEX.get(compactToken(token)) as CanonicalManualOverride | undefined) ?? null;
}

export function lookupSealMaterialAlias(token: string): CanonicalSealMaterial | null {
  return (SEAL_MATERIAL_ALIAS_INDEX.get(compactToken(token)) as CanonicalSealMaterial | undefined) ?? null;
}
