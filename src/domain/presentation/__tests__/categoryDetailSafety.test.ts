...
const HYDRAULIC_ONLY_KEYS = ['function_token', 'connector_token', 'voltage', 'cetop_ng'];

describe('category-specific detail safety', () => {
  const hydraulicId = identifyProduct(HYDRAULIC_CODE, normalizeCode(HYDRAULIC_CODE));
  const pneumaticId = identifyProduct(PNEUMATIC_CODE, normalizeCode(PNEUMATIC_CODE));
  ...
  it('pneumatic evidence rows exclude hydraulic-only fields', () => {
    ...
    for (const label of ['Sürgü / fonksiyon kodu', 'Bobin voltajı', 'Konnektör kodu']) {
      expect(labels).not.toContain(label);
    }
  });
