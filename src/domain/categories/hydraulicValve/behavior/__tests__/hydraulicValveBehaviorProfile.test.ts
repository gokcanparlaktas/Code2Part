...
function buildProfile(input: string) {
  ...
  return buildHydraulicValveBehaviorProfile({
    identification: id,
    attributes: attrs,
  });
}
...
describe("buildHydraulicValveBehaviorProfile", () => {
  ...
  it("Vickers DG4V-3-2A-M-U-H7-60 splits H7 to H=24V DC + tank rating 7", () => {
    const profile = buildProfile("DG4V-3-2A-M-U-H7-60");
    expect(profile.cetopNg).toBe("CETOP 03 / NG6");
    expect(profile.voltage).toBe("24V DC");
    expect(profile.voltageCode).toBe("H");
    expect(profile.requiresCatalogCheck).toBe(true);
...
describe("compareHydraulicValveBehaviorProfiles", () => {
  it("same NG6 + positions + voltage improves compatibility", () => {
    ...
    expect(
      result.comparisons.find((c) => c.label === "CETOP / NG ölçüsü")?.status,
    ).toBe("compatible");
    expect(
      result.comparisons.find((c) => c.label === "Konum sayısı")?.status,
    ).toBe("compatible");
    expect(
    ...
    expect(
      result.comparisons.find((c) => c.label === "Sürgü / fonksiyon kodu")
        ?.status,
    ).toBe("unknownOrCheck");
  });
  ...
  it("different spring arrangement reduces compatibility vs same spool", () => {
    ...
    const sameCompatible = same.comparisons.filter(
      (c) => c.status === "compatible",
    ).length;
    const diffCompatible = diff.comparisons.filter(
      (c) => c.status === "compatible",
    ).length;
    expect(diffCompatible).toBeLessThan(sameCompatible);
    expect(diff.comparisons.find((c) => c.label === "Yay düzeni")?.status).toBe(
      "different",
    );
  });
});
