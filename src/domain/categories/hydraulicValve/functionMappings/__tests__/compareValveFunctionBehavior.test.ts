...
describe('compareValveFunctionBehavior', () => {
  ...
  it('different centerCondition lowers match score vs same-function Yuken pair', () => {
    const source = identifyProduct('DSG-01-3C2-D24-N1-50', normalizeCode('DSG-01-3C2-D24-N1-50'));
    ...
    const differentCenterScore = calculateMatchPercentage(differentCenter).percentage;

    expect(sameFunction.compatible.some((c) => c.label === 'Sürgü / fonksiyon kodu')).toBe(true);
    expect(differentCenter.different.some((c) => c.label === 'Sürgü / fonksiyon kodu')).toBe(true);
